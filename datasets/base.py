...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.split = split
        self.classnames_to_use = [classname] if isinstance(classname, str) else classname
        self.train_val_split = train_val_split
        self.resize = resize
        self.square = square

        # 获取数据路径
        result = self.get_image_data()
        if len(result) == 3:
            self.imgpaths_per_class, self.maskpaths_per_class, self.data_to_iterate = result
        else:
            self.imgpaths_per_class, self.data_to_iterate = result
