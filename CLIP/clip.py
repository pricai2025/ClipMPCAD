...
'''A[开始]：函数开始执行。
B[加载状态字典]：从 checkpoint_path 加载模型的状态字典。
C{是否为旧格式？}：检查状态字典中是否存在 positional_embedding 但模型没有该属性，判断是否为旧格式。
D[转换为新格式]：如果是旧格式，调用 convert_to_custom_text_state_dict 将其转换为新格式。
E[调整位置嵌入]：调用 resize_pos_embed 根据模型的需求调整位置嵌入的大小。
F[加载状态字典到模型]：调用 model.load_state_dict 将状态字典加载到模型中。
G[返回不兼容的键]：返回加载过程中不兼容的键。
H[结束]：函数执行结束。'''
# 用来加载本地模型checkpoint_path
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        # 判断图像大小是否匹配预训练模型的参数，不匹配则重新设置图像大小，调整大小
        # 否则直接加载预训练参数
        ...
        # 图像大小尺寸匹配直接加载预训练模型
        # 这里的model实例化的CLIP模型，在load_openai_model里实现
        # load_openai_model里实现的是build_model_from_openai_state_dict()的返回模型 CLIP
