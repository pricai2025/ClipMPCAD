...
# 与customTextCLIP区分,这个对文本编码器的内部有所设置
class CLIP(nn.Module):
    ...
    def encode_image(self, image, out_layers, normalize: bool = False):
        ...
        return F.normalize(features, dim=-1) if normalize else features  # 是否归一化
...
# 没有视觉编码器内部的细节设置，直接调用的transformer
...
def convert_to_custom_text_state_dict(state_dict: dict):
    ...
    # 如果状态字典包含text_projection，则说明是旧版本状态字典，需要转换
...
def build_model_from_openai_state_dict(
    ...
    # 判断状态字典是否包含visual.proj，来判断，state_dict中是否包含vitTransformer参数
    # 否则认为包含ResNet参数，则加载参数Resnet参数
